...
export const convertDroneObjectToUnified = (
  droneObj: DroneObject,
): UnifiedFlightData => {
...
export const convertUnifiedToDroneObject = (
  unified: UnifiedFlightData,
): DroneObject => {
  return {
    id: unified.id,
    name: unified.name,
...
export const convertWaypointToUnified = (
...
export const convertFlightLogToUnified = (
  log: DroneFlightLog,
): UnifiedFlightData => {
  return {
...
// ミッション距離計算
export const calculateMissionDistance = (
  waypoints: UnifiedFlightData[],
): number => {
...
// ミッション時間推定
export const estimateMissionDuration = (
  waypoints: UnifiedFlightData[],
...
// ドローン飛行ログデータのパース処理
export const parseDroneFlightLogData = (
  csvContent: string,
): DroneFlightLog[] => {
  const lines = csvContent.split("\n").filter((line) => line.trim());
  const headers = lines[0].split(",").map((h) => h.trim());
...
// 建物点検メッシュデータのパース処理
export const parseBuildingInspectionMeshData = (
  csvContent: string,
): MeshVertex[] => {
...
// メッシュデータを三角形に変換
export const convertMeshToTriangles = (
  meshData: MeshVertex[],
): number[][][] => {
...
// 3Dポイントデータの描画
export const add3DPoints = (
  map: maplibregl.Map,
  points: Point3D[],
  sourceId: string = "3d-points",
) => {
...
// 3Dメッシュデータの描画
export const add3DMesh = (
  map: maplibregl.Map,
  meshData: MeshVertex[],
  sourceId: string = "3d-mesh",
) => {
  ...
  try {
    ...
    if (meshData.length > 0) {
      const centerLng =
        meshData.reduce((sum, p) => sum + p.x, 0) / meshData.length;
      const centerLat =
        meshData.reduce((sum, p) => sum + p.y, 0) / meshData.length;
      map.flyTo({
        center: [centerLng, centerLat],
        zoom: 16,
        duration: 2000,
...
// ドローン飛行経路の描画
export const addDroneWaypoints = (
  map: maplibregl.Map,
  waypoints: DroneWaypoint[],
  sourceId: string = "drone-waypoints",
) => {
  console.log(
    "ドローン飛行経路の描画開始:",
    waypoints.length,
  ...
  try {
    ...
    if (waypoints.length > 0) {
      const centerLng =
        waypoints.reduce((sum, p) => sum + p.x, 0) / waypoints.length;
      const centerLat =
        waypoints.reduce((sum, p) => sum + p.y, 0) / waypoints.length;
      map.flyTo({
        center: [centerLng, centerLat],
        zoom: 15,
        duration: 2000,
...
// ドローン飛行ログの描画
export const addDroneFlightLog = (
  map: maplibregl.Map,
  logs: DroneFlightLog[],
  sourceId: string = "drone-flight-log",
) => {
...
export const importDataFromFile = async (
  ...
  type:
    ...
    | "flight-log"
    | "building-inspection"
    | "building-inspection-mesh" = "points",
): Promise<void> => {
  ...
  switch (type) {
    ...
    case "building-inspection":
      ...
      console.log(
        "パースされた建物点検データ:",
        buildingPointData.length,
        "個",
      );
      console.log(
        "最初の3つの建物点検ポイント:",
        buildingPointData.slice(0, 3),
      );
...
// 現在表示されているデータを取得
export const getCurrentDisplayData = (
  map: maplibregl.Map,
): { points: Point3D[]; meshes: MeshVertex[] } => {
  ...
  if (pointsSource) {
    ...
    const geoJsonData =
      typeof pointsData.data === "string"
        ? JSON.parse(pointsData.data)
        : pointsData.data;
  ...
  if (meshSource) {
    ...
    const geoJsonData =
      typeof meshData.data === "string"
        ? JSON.parse(meshData.data)
        : meshData.data;
    if (geoJsonData && geoJsonData.features) {
      ...
      geoJsonData.features.forEach((feature: any, index: number) => {
        if (feature.geometry.type === "Polygon") {
          feature.geometry.coordinates[0].forEach(
            (coord: number[], vertexIndex: number) => {
...
export const clearData = (
  map: maplibregl.Map,
  sourceIds: string[] = ["3d-points", "3d-mesh"],
) => {
    ...
    data.forEach((item: any, index: number) => {
      if (item.longitude && item.latitude) {
        const droneObject: DroneObject = {
          ...
          altitude:
            parseFloat(item.altitude || item.height || item.elevation) || 50,
...
// ドローンオブジェクト描画
export const addDroneObjects = (
  map: maplibregl.Map,
  objects: DroneObject[],
  sourceId: string = "drone-objects",
) => {
  console.log(
    "ドローンオブジェクト描画開始:",
    objects.length,
  ...
  try {
    ...
    if (objects.length > 0) {
      const centerLng =
        objects.reduce((sum, obj) => sum + obj.longitude, 0) / objects.length;
      const centerLat =
        objects.reduce((sum, obj) => sum + obj.latitude, 0) / objects.length;
      map.flyTo({
        center: [centerLng, centerLat],
        zoom: 14,
        duration: 2000,
...
// ドローン軌跡の追加
export const addDroneTrails = (
  map: maplibregl.Map,
  trails: { [droneId: string]: number[][] },
) => {
...
export const exportUnifiedFlightDataToCSV = (
  data: UnifiedFlightData[],
): string => {
...
export const exportUnifiedFlightDataToGeoJSON = (
  data: UnifiedFlightData[],
): string => {
...
// フライト実行結果のJSONエクスポート
export const exportFlightExecutionResultToJSON = (
  result: FlightExecutionResult,
): string => {
...
export const parseUnifiedFlightDataCSV = (
  csvContent: string,
): UnifiedFlightData[] => {
  const lines = csvContent.trim().split("\n");
  if (lines.length < 2) return [];
...
export const parseUnifiedFlightDataGeoJSON = (
  jsonContent: string,
): UnifiedFlightData[] => {
  try {
    ...
    const data: UnifiedFlightData[] = geojson.features.map(
...
// フライトミッションのJSONインポート
export const parseFlightMissionJSON = (
  jsonContent: string,
): FlightMission | null => {
  try {
...
// フライト実行結果のJSONインポート
export const parseFlightExecutionResultJSON = (
  jsonContent: string,
): FlightExecutionResult | null => {
  try {
...
// サンプルデータ生成
export const generateSampleDroneData = (
  center: [number, number] = [139.7454, 35.6586],
): DroneObject[] => {
...
// データクリア（ドローン対応版）
export const clearDroneData = (
  map: maplibregl.Map,
  sourceIds: string[] = ["drone-objects", "drone-trails"],
) => {
  sourceIds.forEach((sourceId) => {
    // レイヤーの削除
