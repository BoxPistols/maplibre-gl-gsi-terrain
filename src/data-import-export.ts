...
export const add3DPoints = (
	...
	try {
		...
		map.addLayer({
			...
			source: sourceId,
			paint: {
				'circle-radius': ['interpolate', ['linear'], ['zoom'], 10, 8, 15, 15, 20, 25],
				'circle-color': [
					'match',
					['get', 'damage_level'],
					0,
...
export const addDroneWaypoints = (
	...
	try {
		...
		map.addLayer({
			...
			source: sourceId,
			paint: {
				'circle-radius': ['interpolate', ['linear'], ['zoom'], 10, 6, 15, 12, 20, 20],
				'circle-color': [
					'match',
					['get', 'action'],
					'takeoff',
...
export const addDroneFlightLog = (
	...
	try {
		...
		map.addLayer({
			...
			source: sourceId,
			paint: {
				'circle-radius': ['interpolate', ['linear'], ['zoom'], 10, 3, 15, 6, 20, 10],
				'circle-color': [
					'interpolate',
					['linear'],
					['get', 'speed'],
...
export const getCurrentDisplayData = (
	...
	if (pointsSource) {
		...
		const geoJsonData =
			typeof pointsData.data === 'string' ? JSON.parse(pointsData.data) : pointsData.data
	...
	if (meshSource) {
		...
		const geoJsonData =
			typeof meshData.data === 'string' ? JSON.parse(meshData.data) : meshData.data
		if (geoJsonData && geoJsonData.features) {
...
export const exportFlightMissionToKML = (mission: FlightMission): string => {
	...
	const pathCoordinates = waypoints
		.map(wp => `${wp.position.longitude},${wp.position.latitude},${wp.position.altitude}`)
		.join(' ')
