...
const completePolygon = () => {
  ...
  showToast(
    `多角形「${polygonObject.name}」を作成しました (面積: ${area.toFixed(0)}㎡)`,
    "success",
  );
};
...
const selectObject = (lngLat: maplibregl.LngLat) => {
  ...
  if (closestObject) {
    ...
    showToast(
      `「${(closestObject as DroneObject).name}」を選択しました`,
      "info",
    );
    return true;
  } else {
...
const dragObject = (lngLat: maplibregl.LngLat) => {
  ...
  if (selectedObject.type === "polygon") {
    ...
    if (polygonData.geometry && polygonData.geometry.coordinates) {
      polygonData.geometry.coordinates[0] =
        polygonData.geometry.coordinates[0].map((coord: [number, number]) => [
          coord[0] + deltaLng,
          coord[1] + deltaLat,
        ]);
    }
...
const setupEventHandlers = () => {
  ...
  document.getElementById("loadDroneData")?.addEventListener("click", () => {
    if (!sampleDataLoaded) {
      ...
      updateStatus(
        `東京タワー点検データ読み込み完了: ${sampleData.length}オブジェクト`,
      );
  ...
  document.getElementById("toggleDrawMode")?.addEventListener("click", () => {
    ...
    showToast(
      drawMode ? "描画モードを有効にしました" : "描画モードを無効にしました",
      "info",
    );
  });
  ...
  document
    ...
    ?.addEventListener("click", () => {
      ...
      if (button) {
        button.textContent = polygonDrawingMode
          ? "多角形作成停止"
          : "多角形作成";
      }
  ...
  document.getElementById("toggleEditMode")?.addEventListener("click", () => {
    ...
    showToast(
      editMode ? "編集モードを有効にしました" : "編集モードを無効にしました",
      "info",
    );
  });
      ...
      if (file) {
        try {
          ...
          if (importedObjects.length > 0) {
            ...
            updateDisplay();
            updateStatus(
              `CSV読み込み完了: ${importedObjects.length}個のオブジェクト`,
            );
            showToast(
      ...
      if (file) {
        try {
          ...
          if (importedObjects.length > 0) {
            ...
            updateDisplay();
            updateStatus(
              `GeoJSON読み込み完了: ${importedObjects.length}個のオブジェクト`,
            );
            showToast(
  ...
  document.getElementById("exportGeoJSON")?.addEventListener("click", () => {
    if (loadedObjects.length > 0) {
      ...
      downloadFile(
        geojson,
        "tokyo_tower_drone_data.geojson",
  ...
  document.getElementById("toggle3D")?.addEventListener("click", () => {
    ...
    showToast(
      is3D ? "3D表示に切り替えました" : "2D表示に切り替えました",
      "info",
    );
  });
  ...
  document.getElementById("toggleLog")?.addEventListener("click", () => {
    ...
    console.log(
      "現在のFlightLog表示状態:",
      flightLog?.classList.contains("visible"),
    );
      ...
      if (file) {
        try {
          ...
          if (importedData.length > 0) {
            ...
            const convertedObjects = importedData.map((data) =>
              convertUnifiedToDroneObject(data),
            );
            loadedObjects.push(...convertedObjects);
            updateDisplay();
            updateStatus(
              `フライトデータ読み込み完了: ${importedData.length}個のオブジェクト`,
            );
            showToast(
      ...
      if (file) {
        try {
          ...
          if (mission && mission.waypoints && mission.waypoints.length > 0) {
            ...
            const waypointObjects: DroneObject[] = mission.waypoints.map(
              (waypoint, index) => ({
            ...
            map.flyTo({
              center: [
                firstWaypoint.position.longitude,
                firstWaypoint.position.latitude,
              ],
              zoom: 16,
              duration: 2000,
            });
  ...
  // サンプルフライトデータ読み込み
  document
    .getElementById("loadSampleFlightData")
    ?.addEventListener("click", async () => {
      try {
        ...
        if (importedData.length > 0) {
          ...
          updateDisplay();
          updateStatus(
            `サンプルフライトデータ読み込み完了: ${importedData.length}個のオブジェクト`,
          );
          showToast(
  ...
  // サンプル軌跡データ読み込み
  document
    .getElementById("loadSampleTrajectory")
    ?.addEventListener("click", async () => {
      try {
        ...
        if (importedData.length > 0) {
          ...
          updateDisplay();
          updateStatus(
            `サンプル軌跡データ読み込み完了: ${trajectoryData.length}個の軌跡点`,
          );
          showToast(
  ...
  document.getElementById("exportFlightData")?.addEventListener("click", () => {
    if (loadedObjects.length > 0) {
      ...
      const unifiedData = loadedObjects.map((obj) =>
  ...
  document.getElementById("exportMission")?.addEventListener("click", () => {
    if (loadedObjects.length > 0) {
      ...
      if (flightTypeObjects.length > 0) {
        try {
          const unifiedFlightData: UnifiedFlightData[] = flightTypeObjects.map(
            (obj) => convertDroneObjectToUnified(obj),
          );
      ...
      if (file) {
        try {
          ...
          if (trajectoryData.length > 0) {
            ...
            updateDisplay();
            updateStatus(
              `軌跡データ読み込み完了: ${trajectoryData.length}個の軌跡点`,
            );
            showToast(
...
const exportFlightPlan = () => {
  const planData: FlightPlanData = {
    ...
    totalDuration: currentFlightPlan.reduce(
...
const importFlightPlan = () => {
  ...
  input.onchange = (e) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        try {
          const planData: FlightPlanData = JSON.parse(
