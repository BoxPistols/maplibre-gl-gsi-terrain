...
const DroneDataSystem: React.FC<{ className?: string }> = ({ className = '' }) => {
    ...
    useEffect(() => {
        ...
        map.current = new maplibregl.Map({
            ...
            style: {
                version: 8,
                sources: {
                    gsi: { type: 'raster', tiles: ['https://cyberjapandata.gsi.go.jp/xyz/seamlessphoto/{z}/{x}/{y}.jpg'], attribution: '地理院タイル' },
                    gsidem: { type: 'raster-dem', tiles: ['gsidem://https://cyberjapandata.gsi.go.jp/xyz/dem_png/{z}/{x}/{y}.png'], tileSize: 256, maxzoom: 14 },
                    'objects-3d': { type: 'geojson', data: { type: 'FeatureCollection', features: [] } },
                    'drone-trails': { type: 'geojson', data: { type: 'FeatureCollection', features: [] } }
                },
        ...
        map.current.on('error', (e) => { console.error('MapLibreエラー:', e); setStatus('マップエラー発生'); });
        return () => { if (map.current) map.current.remove(); };
    }, [gsidem2terrainrgb]);

    // (マップレイヤー設定、イベント設定、オブジェクト表示更新は変更なし)
    const setupMapLayers = useCallback(() => { /* ... */ }, []);
    const setupMapEvents = useCallback(() => { /* ... */ }, [drawMode]);
    const updateDisplay = useCallback(() => { /* ... */ }, [loadedObjects]);
    useEffect(() => { updateDisplay(); }, [loadedObjects, updateDisplay]);
    const showObjectInfo = useCallback((e: maplibregl.MapMouseEvent) => { /* ... */ }, []);
    const addObjectAtLocation = useCallback((lngLat: maplibregl.LngLat) => { /* ... */ }, [loadedObjects.length]);
            ...
            try {
                ...
                if (file.name.endsWith('.json')) {
                    ...
                    if (data.phases && Array.isArray(data.phases)) {
                        ...
                        map.current?.flyTo({ center: firstPhase.position, zoom: 15, pitch: 70 });
                ...
                const newObjects = objects.filter(newObj => 
                    !loadedObjects.some(existing => 
    ...
    const executeFlightPlan = useCallback(() => {
        if (!flightPlan || currentPhaseIndex !== -1) {
            setStatus(currentPhaseIndex !== -1 ? 'フライト実行中です' : '実行するフライトプランがありません');
            return;
        }
    ...
    useEffect(() => {
        ...
        setLoadedObjects(prev => prev.map(obj => 
            obj.id === 'flight_drone_01' 
                ? { ...obj, longitude: phase.position[0], latitude: phase.position[1], altitude: phase.position[2] } 
                : obj
        ));
    ...
    const handleDrop = useCallback((e: React.DragEvent) => { e.preventDefault(); setDragOver(false); if (e.dataTransfer.files) handleFiles(e.dataTransfer.files); }, [handleFiles]);
    const clearAllData = useCallback(() => { /* ... */ }, [/* ... */]);
    const toggle3D = useCallback(() => { /* ... */ }, [is3D]);
    const enableDrawMode = useCallback(() => { /* ... */ }, [drawMode]);
    const removeObject = useCallback((objectId: string) => { /* ... */ }, []);
    const focusOnObject = useCallback((objectId: string) => { /* ... */ }, [loadedObjects]);
    const stats = React.useMemo(() => { /* ... */ }, [loadedObjects]);
    const sortedObjects = React.useMemo(() => { /* ... */ }, [loadedObjects]);
    ...
    return (
        <div className={`drone-data-system ${className}`} style={{ height: '100vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ height: '350px', overflowY: 'auto', padding: '20px', background: '#f5f5f5', borderTop: '1px solid #ddd' }}>
                <h3>✈️ フライトプランナー</h3>
                
                <div style={{ margin: '10px 0', padding: '10px', background: '#e8f4f8', borderRadius: '5px', fontSize: '14px' }}>
                    {status}
                </div>
                ...
                <div 
                    ...
                    style={{ border: `2px dashed ${dragOver ? '#007cba' : '#ccc'}`, background: dragOver ? '#e3f2fd' : 'transparent', padding: '20px', textAlign: 'center', margin: '10px 0', cursor: 'pointer', transition: 'all 0.3s', borderRadius: '5px' }}
                >
                    📂 フライトプラン(JSON)や他のデータ(CSV)をドロップ
                </div>
                <input ref={fileInputRef} type="file" multiple accept=".csv,.json,.geojson" style={{ display: 'none' }} onChange={(e) => e.target.files && handleFiles(e.target.files)} />
                
                <div style={{ marginBottom: '15px' }}>
                    <button onClick={executeFlightPlan} disabled={!flightPlan || currentPhaseIndex !== -1} style={buttonStyle}>🚁 フライトプラン開始</button>
                    <button onClick={stopFlightPlan} disabled={currentPhaseIndex === -1} style={buttonStyle}>⏹️ フライト停止</button>
                    ...
                    <div>
                        ...
                        <ul>
                            {flightPlan.phases.map((phase, index) => (
                                <li key={index} style={{ fontWeight: index === currentPhaseIndex ? 'bold' : 'normal', color: index < currentPhaseIndex ? 'gray' : 'black' }}>
